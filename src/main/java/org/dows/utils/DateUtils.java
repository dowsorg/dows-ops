...
public class DateUtils {
     ...
     * @return 201908
     */
    public static String getYearAndMonthByString() {
        Date date = new Date();
    ...
    public static boolean CurrentTimeIsBelongTimeZone(String currentTime, String beginTimeFormat, String endTimeFormat) {
        Boolean result = false;
        SimpleDateFormat formatOrgin = new SimpleDateFormat("yyyy-MM-dd HH:mm");

        SimpleDateFormat formatTarget = new SimpleDateFormat("HH:mm");
        try {
            Date origin = formatOrgin.parse(currentTime);
            String target = formatTarget.format(origin);
            Date nowTime = formatTarget.parse(target);
            Date beginTime = formatTarget.parse(beginTimeFormat);
            Date endTime = formatTarget.parse(endTimeFormat);
            if (belongCalendar(nowTime, beginTime, endTime)) {
                result = true;
            } else {
                result = false;
            }
    ...
    public static String getIndicateDate(String HourAndMinutes) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");

//        Date parse = format.parse("08:30");
//        System.out.println(parse);
        Calendar instance = Calendar.getInstance();
        Date currentCalendar = instance.getTime();
        String format1 = format.format(currentCalendar);
        String formatConcat = format1 + " ";
        formatConcat = formatConcat.concat(HourAndMinutes);
        return formatConcat;
//        System.out.println(formatConcat);
    }


    public static Float dateSubtractionTransferHours(String beginTime, String endTime) {
        long nd = 1000 * 24 * 60 * 60;
        long nh = 1000 * 60 * 60;
        long nm = 1000 * 60;
        Float result;
        long subStraction = dateSubtraction(beginTime, endTime);
        float hour = subStraction % nd / nh;
        float minutes = (float) subStraction % nd % nh / nm / 60;
        result = hour + minutes;
    ...
    public static void main(String[] args) throws ParseException {
        ...
        System.out.println(DateUtils.getYearAndMonthByString());
