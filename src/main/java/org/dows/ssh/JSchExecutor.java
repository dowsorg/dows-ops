...
import org.dows.utils.DateUtils;
...
/**
 * @description: Java操作sftp对象类，用来推送文件到sftp服务器
 */
@SuppressWarnings("all")
public class JSchExecutor {
    private static Logger logger = LoggerFactory.getLogger(JSchExecutor.class);
...
//      此处要关闭文件流否则传输完成后，无法删除原文件
        input.close();
        logger.info("File upload successfully: {" + uploadFile.getPath() + "}");
    }
    ...
    public void hashRemotePathOrMkdirAndCDRemotePath(String remotePath) throws SftpException {
        ...
        if (hasPath(remotePath, channelSftp)) {
            logger.info("remotePath is exist！ all files will be push to /" + remotePath);
            logger.info("found pathHome {" + remotePath + " }");
            return;
        } else {
//             远程文件路径不存在，创建该目录
            logger.info("remotePath(" + remotePath + ") is  not exist!");
            logger.info("创建该目录:/" + remotePath);
...
//        1.2 判断sftp服务器有没有年月份目录
        ...
        try {
            if (hasPath) {
                logger.info("目标目录:" + PathLikeCurrentYearMonth + "存在");
//       1.3 路径存在cd到当前月目录下
                logger.info("Cd 》》》》》" + PathLikeCurrentYearMonth);
                channelSftp.cd(PathLikeCurrentYearMonth);
                logger.info("当前目录在:" + channelSftp.pwd());
            } else {
                logger.info("目标目录不存在" + PathLikeCurrentYearMonth);
//        2.当前目录没有月目录like 202310，执行命令 mkdir `Date +%Y%m%d`

                logger.info("创建目标目录 mkdir " + PathLikeCurrentYearMonth);
